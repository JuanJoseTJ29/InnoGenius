...
@section('content_header')
    ...
    <h1 class="m-0 text-dark">Administrar mis Ideas</h1>
    @if($busqueda)
...
<div class="col-sm-13">
    ...
    <a href="{{ route('service.create') }}" class="btn btn-success -item">Registrar Idea <i class="fas fa-plus"></i></a>
    </ol>
    </div>
    <br/>
    <br/>
    ...
    <table class="table table-bordered table-striped dataTable table-hover text-center" >
        ...
        <thead class="thead-light">
            <tr>
                ...
                <th>Apoyar  <i class="fas fa-hand-holding-usd"></i></th>
            ...
            <tr>
                ...
                <td>
                    ...
                    <form method="post" action="{{ route('service.destroy', $service) }}" style="display:inline">
                        {{csrf_field()}}
                        {{method_field('DELETE')}}
                        <button class="btn btn-danger" type="submit" onclick="return confirm('¿Estas seguro que deseas Borrar tu idea?');">Borrar</button>
                    </form>
                </td>
            </tr>
            @endforeach
